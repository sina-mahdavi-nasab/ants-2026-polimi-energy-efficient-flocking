function [eff, dist_travelled, average_batt, collision_counter] = simulation_free_global_mod_2_LJ(rules,seed)
%% parameter setting
...
set(gcf, 'Position', [0 0 3010 1596]);
[yVals, xVals, powerVals] = ...
        RayTraceCircularRobots_mex(agents, wind_rad, Uinf, xRange, yRange, Nx, Ny, useGPU);
plot_all(ax,agents,robot_rad,yVals, xVals, powerVals,t,v)


% LJ "spacing" parameters
r0          = 0.70;          % desired inter-robot spacing [m]
sigma       = r0 / sqrt(2);  % standard LJ equilibrium relation
epsilon     = 0.5;           % depth (force scale). Try 0.05–0.15
r_cut       = 3;             % cutoff for LJ summation
r_min       = 0.0   ;          % avoid singularity if r ~ 0

% Alignment and goal terms
R_align     = 1.5;           % neighbor radius for alignment
k_align     = 0;           % heading alignment gain (rad/s)
k_goal      = 3;          % pull to the left (−x)
K1          = 0.05;           % force → speed scaling
K2          = 0.5;
U           = 0.005;
v_lin_max   = 0.20;          % [m/s]
w_max       = pi/5;          % [rad/s]
...
while ~batteryEmpty
    ...
    exr = ex.*repmat(cos(th),[1 n_agents]) + ey.* repmat(sin(th),[1 n_agents]);
    eyr = ey.*repmat(cos(th),[1 n_agents]) - ex.* repmat(sin(th),[1 n_agents]);
    ...
    sig_over_r6  = (sigma.^2) ./ (R.^2 + 1e-6*ones(n_agents,n_agents));   % safe denom
    sig_over_r12 = sig_over_r6.^2;
    Fmag = 8*epsilon .* (2*sig_over_r12 - sig_over_r6) ./ (R + 1e-6*ones(n_agents,n_agents));
    ...
    Th1 = repmat(th.',[n_agents 1]);
    Th2 = repmat(th, [1, n_agents]);
    H_x = align_mask.*cos(Th1);
    H_y = align_mask.*sin(Th1);

    Hb_x = H_x.*cos(Th2) + H_y .* sin(Th2);
    Hb_y = H_y.*cos(Th2) - H_x .* sin(Th2);
    Fa_x = sum(Hb_x,2);
    Fa_y = sum(Hb_y,2);

    A    = max(sqrt(Fa_x.^2 + Fa_y.^2), 1e-9);
    Fa_x = Fa_x ./ A;
    Fa_y = Fa_y ./ A;
    ...
    u   = max(-0.2,min(K1*F_x +  U, 0.2));
    w   = max(-pi/5,min(K2*F_y,pi/5));

    

    for i = 1:n_agents
        
        vel(i,1) = u(i);     % linear speed
        vel(i,2) = w(i);     % angular rate
    end
...
batt_drain = max(sum(abs(wheels_vel),2)/8-dottprod,0.10)*dt ;           %power of velocity - work by wind, max to have a minimum power usage when not moving or moving with the wind
